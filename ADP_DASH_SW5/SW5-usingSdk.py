        ...
        if entry[2] == 1.0:
            ...
            for k,v in rate:
                if v < -1 or v > 1:
                    ...
                    if StartTime == 0:
                        StartTime = k
                    elif k == rate.get_max_key() and StartTime != 0:
                        TotalReadyTime += rate.get_datapoint(k, LQ.NEAREST_SMALLER)[0] - StartTime
    print(TotalReadyTime)


def calculateTotalPurgeTime(stateList):
    ...
    for entry in stateList:
        if entry[2] == 1.0:
            ...
            i=0
            for k, v in data:
                if i == len(data) and Startkey != 0:
                    EndKey = k
                    tempStateList.append(tuple((Startkey, EndKey)))
                    Startkey = 0
                elif v < 1000 and Startkey == 0:
                    Startkey = k
                elif v < 1000 and Startkey != 0:
                    i += 1
                    continue
